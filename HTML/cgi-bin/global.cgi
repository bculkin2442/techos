...
if ($form{'id'}) {
	$sitekeys = "/var/lib/gtags/sitekeys/" . $form{'id'};
...
if ($^O eq 'MSWin32') {
	open(PIPE, '/usr/bin/global' . " --result=ctags-xid $flags $pattern |");
} else {
	open(PIPE, "-|") || exec '/usr/bin/global', '--result=ctags-xid', $flags, $pattern;
	if ($?) {
